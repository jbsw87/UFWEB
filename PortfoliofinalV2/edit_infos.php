...
<?php
//Je verifie si lutilisateur est connecte
	...
	if(isset($_POST['username'], $_POST['password'], $_POST['passverif'], $_POST['email'], $_POST['avatar']))
	{
		...
		if($_POST['password']==$_POST['passverif'])
		{
			...
			if(strlen($_POST['password'])>=6)
			{
				...
				{
					//J'echape les variables pour pouvoir les mettre dans une requette SQL
					...
					//Je verifie si le pseudo a ete modifie pour un autre et que celui-ci n'est pas deja utilise
					if($dn['nb']==0 or $_POST['username']==$_SESSION['username'])
					{
						//Je modifie les informations de lutilisateur avec les nouvelles
						if(mysql_query('update users set username="'.$username.'", password="'.$password.'", email="'.$email.'", avatar="'.$avatar.'" where id="'.mysql_real_escape_string($_SESSION['userid']).'"'))
						{
							...
							$form = false;
							//Je supprime les sessions username et userid au cas ou il aurait modifie son pseudo
...
<?php
	...
	if($form)
	{
		//J'affiche un message si il y a lieu
		if(isset($message))
		{
			echo '<strong>'.$message.'</strong>';
		}
		//Si le formulaire a deja ete envoye je recupere les donnes que lutilisateur avait deja insere
		...
		{
			//Sinon, j'affiche les donnes a partir de la base de donnee
