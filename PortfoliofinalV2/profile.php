...
<body>
	...
	<div class="content">
		<?php
		//Je verifie que l'identifiant de lutilisateur est defini
		if (isset($_GET['id'])) {
			$id = intval($_GET['id']);
			//Je verifie que l'utilisateur existe
			$dn = mysql_query('select username, email, avatar, signup_date from users where id="' . $id . '"');
			if (mysql_num_rows($dn) > 0) {
				$dnn = mysql_fetch_array($dn);
				//J'affiche les donnees de lutilisateur
				?>
				...
				<table style="width:500px;">
					<tr>
						...
						<td class="left">
							...
							Cet utilisateur s'est inscrit le <?php echo date('d/m/Y', $dnn['signup_date']); ?>
						</td>
					</tr>
				</table>
			<?php
